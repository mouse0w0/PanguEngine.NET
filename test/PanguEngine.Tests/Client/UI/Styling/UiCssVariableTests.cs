...
public sealed class UiCssVariableTests
{
    ...
    [Theory]
    [InlineData("Panel:hover { --n: 4px; } Panel { padding: var(--n, 0); }")]
    [InlineData("Panel:hover { --n: 4px; } Panel { --alias: var(--n); padding: var(--alias, 0); }")]
    [InlineData("Panel:hover { --n: 4px; } Panel { --a: 2px; padding: var(--a, var(--n)); }")]
    public void PseudoVariableDependenciesCannotAffectLayout(string css)
    {
        var panel = new Panel();
        var screen = new UiScreen(panel);
        var error = Assert.Throws<UiStyleParseException>(() => screen.SetStyleSheets([UiStyleSheet.Parse(css)]));
        Assert.Equal(UiStyleParseError.InvalidValue, error.Error);
        Assert.Contains("hover", error.InnerException!.Message);
    }

    [Fact]
    public void PseudoConsumerCannotSetLayoutThroughStaticVariable()
    {
        var panel = new Panel();
    ...
    [Fact]
    public void InheritedPseudoVariableCannotAffectChildLayout()
    {
        ...
        var error = Assert.Throws<UiStyleParseException>(() => screen.SetStyleSheets([UiStyleSheet.Parse("""
            ...
            """)]));
        Assert.Equal(UiStyleParseError.InvalidValue, error.Error);
        Assert.Contains("#root:hover", error.InnerException!.Message);
    }
